class TrainerDashboard {
    ...
    initializeNavigation() {
        ...
        const handleTabAndSectionClick = (clickedTab) => {
    ...
    resetForm(form) {
        ...
        fileLabels.forEach(label => {
            ...
            if (text.includes('Ready to Upload')) {
                label.firstChild.textContent = label.getAttribute('data-original-text') || 'Select File';
            }
        });
    }
    ...
    addEnrollmentEventListeners() {
        ...
        acceptBtns.forEach(btn => {
            btn.addEventListener('click', () => this.processEnrollment(btn.dataset.id, 'accept', btn));
        });

        rejectBtns.forEach(btn => {
            btn.addEventListener('click', () => this.processEnrollment(btn.dataset.id, 'reject', btn));
        });
    }
    ...
    addStateButtonListeners() {
        ...
        stateBtns.forEach(btn => {
            btn.addEventListener('click', () => this.toggleSelfEnrollment(btn.dataset.id, btn));
        });
    }
