class StudentDashboard {
  ...
  init() {
    ...
    this.initializeActivitySubmission();
  ...
  initializeActivitySubmission() {
    document.addEventListener("click", async (e) => {
      if (e.target.id === "submitActivity") {
        e.preventDefault();
        ...
        const activityId = row.querySelector('input[name="activity_id"]').value;
        const activityType = row.querySelector(
          'input[name="activity_type"]'
        ).value;
        const courseId = row.querySelector('input[name="course_id"]').value;
  ...
  async submitActivity(activityId, activityType, courseId, file) {
    ...
    try {
      const response = await fetch("../php/submitActivity.php", {
        method: "POST",
        body: formData,
      });
      ...
      alert(
        "An error occurred while submitting the activity. Please try again."
      );
    }
  }
